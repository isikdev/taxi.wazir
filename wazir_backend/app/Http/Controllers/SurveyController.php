...
class SurveyController extends Controller
{
    ...
    /**
     * Переход к шагу 8 - загрузка документов
     */
    public function showStep8()
    {
        ...
        // Проверяем, может ли пользователь перейти на этот шаг
        if (!$this->canAccessStep(8)) {
            return redirect()->route('driver.survey.step' . session('max_survey_step', 1));
        }
        
        $this->saveCurrentStep(8);
        return view('driver.survey.step8');
    }

    /**
     * Обрабатывает шаг 8 - сохранение документов
     */
    public function processStep8(Request $request)
    {
        ...
        $request->validate([
            'passport_front' => 'required|image|max:5120', // 5MB
            'passport_back' => 'required|image|max:5120',
            'driving_license_front' => 'required|image|max:5120',
            'driving_license_back' => 'required|image|max:5120',
            // Новые обязательные поля для фотографий автомобиля
            'car_front' => 'required|image|max:5120',
            'car_back' => 'required|image|max:5120',
            'car_left' => 'required|image|max:5120', 
            'car_right' => 'required|image|max:5120',
            // Новые обязательные поля для фотографий салона
            'interior_front' => 'required|image|max:5120',
            'interior_back' => 'required|image|max:5120',
        ]);

        // Временно сохраняем файлы в папку storage/app/temp и сохраняем пути в сессии
        $driverId = session('driver_id');
        $tempPath = "driver_documents/driver_{$driverId}";
        ...
        $files = [
            ...
            // Добавляем новые поля для фотографий автомобиля
            'car_front' => 'car_front',
            'car_back' => 'car_back',
            'car_left' => 'car_left',
            'car_right' => 'car_right',
            // Добавляем новые поля для фотографий салона
            'interior_front' => 'interior_front',
            'interior_back' => 'interior_back'
        ];
        
        foreach ($files as $fileKey => $dbField) {
            // Если $fileKey - это строка, значит у нас есть отображение имени файла на поле БД
            $inputKey = $fileKey;
            ...
            if ($request->hasFile($inputKey)) {
                ...
                $documentPaths[$dbField] = $path;
                
                // Сразу обновляем запись водителя с путями к документам
                $driver = Driver::find($driverId);
                if ($driver) {
                    $driver->{$dbField} = $path;
                    $driver->save();
        ...
        // Перенаправляем на страницу проверки (redirect_to_complete не нужен, т.к. это последний шаг)
        return redirect()->route('driver.survey.complete');
    }
    ...
    public function complete()
    {
        ...
        $data = [
            ...
            // Документы
            'documents' => [
                'passport_front' => $driver->passport_front ?? null,
                'passport_back' => $driver->passport_back ?? null,
                'license_front' => $driver->license_front ?? null,
                'license_back' => $driver->license_back ?? null,
                'car_front' => $driver->car_front ?? null,
                'car_back' => $driver->car_back ?? null,
                'car_left' => $driver->car_left ?? null,
                'car_right' => $driver->car_right ?? null,
                'interior_front' => $driver->interior_front ?? null,
                'interior_back' => $driver->interior_back ?? null
            ]
        ];
    ...
    public function submitApplication()
    {
        ...
        // Обновляем статус анкеты и данные водителя
        $driver->update([
        ...
        // Создаем или обновляем запись о транспортном средстве
        $driver->vehicle()->updateOrCreate(
            ['driver_id' => $driverId],
            [
                'vehicle_brand' => session('car_brand'),
                'vehicle_model' => session('car_model'),
                'vehicle_color' => session('car_color'),
                'vehicle_year' => session('car_year'),
                'vehicle_state_number' => session('license_plate'),
                'chassis_number' => session('vin'),
                'sts' => session('sts'),
                'stir' => session('body_number'),
            ]
        );
        
        // Явно устанавливаем статус 'submitted' и сохраняем еще раз для обеспечения сохранения
